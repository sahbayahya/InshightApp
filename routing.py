...
def get_path_length(path):
    return np.sum(geocalc(path[1:,0], path[1:,1],
                          path[:-1,0], path[:-1,1]))  
sg= pickle.load(open("/Users/sahba/Dropbox/DSInsightProject/app/areas/Manhattanmap.p", "rb" ))
def routing(pos0, pos1):
    ...
    roads = pd.DataFrame([sg.edge[path[i]][path[i + 1]] for i in range(len(path) - 1)])
    print roads 
